...
export const useUploadNewLabellersStore =
  create(
    subscribeWithSelector<UploadNewLabellersState>(
        (set, get) => ({
          ...
          onNextStepHandler: async () => {
            ...
            const newActiveStep = (() => {
              const updatedStep = (activeStep < TOTAL_UPLOAD_STEPS ? activeStep + 1 : activeStep)
              if (callback && typeof callback === 'function' && callbackResultSuccess) return updatedStep
              else if (callback && typeof callback === 'function' && !callbackResultSuccess) return activeStep
          ...
          onIngredientSentenceEntryHandler: async () => {
            ...
            const parseNewLabellerItemsForUploadApi = useTabLabellerStore.getState().parseNewLabellerItemsForUploadApi
          ...
          setState: (values: Partial<AccessibleUploadNewLabellersState>) => set({ ...values }),
          ...
          parsedSentencesHandler: {
            ...
            append: (...items: ParsedSentenceEditable[]) => set(({ parsedSentences }) => ({ parsedSentences: [...parsedSentences, ...items] })),
            prepend: (...items: ParsedSentenceEditable[]) => set(({ parsedSentences }) => ({ parsedSentences: [...items, ...parsedSentences] })),
            insert: (index: number, ...items: ParsedSentenceEditable[]) =>
              set(({ parsedSentences }) =>
                ({ parsedSentences: [...parsedSentences.slice(0, index), ...items, ...parsedSentences.slice(index)] })
              ),
            pop: () =>
              set(({ parsedSentences }) => {
            ...
            ,
            apply: (fn: (item: ParsedSentenceEditable, index?: number) => ParsedSentence) =>
              set(({ parsedSentences }) =>
                ({ parsedSentences: parsedSentences.map((item, index) => fn(item, index)) })
              )
            ,
            applyWhere: (
              ...
              set(({ parsedSentences }) =>
                ({ parsedSentences: parsedSentences.map((item, index) => (condition(item, index) ? fn(item, index) : item)) })
              )
            ,
            remove: (...indices: number[]) =>
              set(({ parsedSentences }) => ({ parsedSentences: parsedSentences.filter((_, index) => !indices.includes(index)) }))
            ,
            ...
            ,
            setItemProp: <K extends keyof ParsedSentenceEditable, U extends ParsedSentenceEditable[K]>(
              index: number,
              prop: K,
              value: U
            ) =>
            ...
            filter: (fn: (item: ParsedSentenceEditable, i: number) => boolean) => {
              set(({ parsedSentences }) => ({ parsedSentences: parsedSentences.filter(fn) }));
            },
          },
          ...
          bulkUploadApi: async () => {
            ...
            const fetchAvailableSourcesApi = useTabLabellerStore.getState().fetchAvailableSourcesApi
