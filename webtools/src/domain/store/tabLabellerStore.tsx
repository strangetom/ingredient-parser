...
// {{{INTERNAL}}}
import { InputTabLabeller, InputSettings, ParsedTabLabller, ParsedSentence, ParsedSentenceEditable, LabellerCategory } from "../types";
...
export const useTabLabellerStore =
  create(
    subscribeWithSelector<TabLabellerState>(
        (set, get) => ({
          loading: false,
          editing: false,
          preuploading: false,
          ...
          updateInputSettings: (partial: Partial<InputSettings>) =>
            set(
              ({ input }) =>({ input: { ...input, settings: { ...input.settings, ...partial } } })
            )
          ,
          parsed: null,
          ...
          getLabellerSearchApi: async (opts?: TabLabellerInputProvided) => {
            ...
            if (input && input.sentence.length !== 0) {
              ...
              const trimmed = input.sentence.length === 2 && /(\*\*|~~)/.test(input.sentence) ? input.sentence : input.sentence.trim()

              await fetch(
                constructEndpoint({ path: "labellerSearch" }), {
                method: 'POST',
          ...
          editLabellerItemsApi: async () => {
            ...
            const result  = await fetch(
              constructEndpoint({ path: "labellerSave" }), {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  "edited": sentences.filter(({ edited }) => edited).map(({ edited, removed, ...others }) => ({ ...others })),
                  "removed": sentences.filter(({ removed }) => removed).map(({ edited, removed, ...others }) => ({ ...others }))
                })
            })
              .then(response => {
                if (response.ok) return response.json()
