...
export const constructEndpoint = ({ path, hasPort = true, isWebSocket = false}: ConstructEndpointOpts): string => {
  ...
  try {
    ...
    const urlWithPort = base + ":" + (isWebSocket ? Endpoints.websocketPort : Endpoints.basePort) + Endpoints.paths[path]
