...
import { ActionIcon, Anchor, Box, Button, Code, Flex, Group, JsonInput, Loader, LoadingOverlay, Menu, MultiSelect, NumberInput, Switch, Text } from "@mantine/core"
...
// {{{ASSETS}}}
import { IconChevronDown, IconChevronUp, IconCode, IconX } from "@tabler/icons-react"
...
export function ButtonRunModelGridSearch() {
  ...
  const [prettyJsonOpened, { open: openPrettyJson, close: closePrettyJson }] = useDisclosure(false);
  ...
  const seedInput = (
    <Group gap="xs">
      ...
      <Button
        variant="light"
        onClick={() => {
          updateInputGridSearch({ seed: Math.floor(Math.random() * 1_000_000_001) })
        }}
      >
        Generate
      </Button>
  ...
  const algosInput = (
    <div>
      <Box style={{ position: "relative"}}>
        ...
        {
          !jsonError &&
          <Box style={{ position: "absolute", bottom: "var(--xsmall-spacing)", right: "var(--xsmall-spacing)" }}>
       ...
       <Text fz="xs">View CRFSuite's <Anchor target="_blank" fz="inherit" href="https://www.chokkan.org/software/crfsuite/manual.html">hyper-parameters documentation</Anchor></Text>
    </div>
  )
  ...
  const combineLabelsFlagSwitch = (
    <Switch
      ...
      onChange={(event) => {
        updateInputGridSearch({ combineNameLabels: event.currentTarget.checked })
      }}
    />
  )
        ...
        <Menu shadow="md" keepMounted={false} position="bottom-end" width={500} closeOnItemClick={false} offset={8} opened={opened} onChange={setOpened} trigger="click" menuItemTabIndex={0} loop={false} trapFocus={true}>
          ...
          <Menu.Dropdown>
            <LoadingOverlay
              ...
              loaderProps={{
                style: { position: "relative", height: "100%", width: "100%", padding: "var(--medium-spacing)"  },
                children: (
                  <div style={{  position: "relative", height: "100%", width: "100%"  }}>
                    <Code block style={{ height: "100%", width: "100%" }}>
                      {!jsonError && JSON.stringify(JSON.parse(inputGridSearch.algosGlobalParams), null, 2)}
                    </Code>
                    <Box style={{ position: "absolute", top: "var(--xsmall-spacing)", right: "var(--xsmall-spacing)" }}>
                      <ActionIconTooltipped
                        text="Exit"
                        iconography={IconX}
              ...
              zIndex={99999}
              overlayProps={{ blur: 0, backgroundOpacity: 0.9, color: 'var(--bg-1)', center: false }}
              styles={{ root: { display: 'block', align: undefined, justifyContent: undefined }}}
            />
