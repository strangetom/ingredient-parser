...
import { Box, Center, Combobox, Group, Input, InputBase, Loader, ScrollArea, Text, Image, useCombobox, Paper, TextInput, Flex, TextProps, SegmentedControl, Avatar } from "@mantine/core"
import { useShallow } from "zustand/react/shallow";
// {{{INTERNAL}}}
import { useUploadNewLabellersStore, collectionPublisherSources, CollectionPublisherSource, useTabLabellerStore } from "../../../../../domain";
// {{{STYLES}}}
...
export function SelectPublisherSource() {
  ...
  const coerceSourcesToComboboxOptions = useCallback(() =>
    availablePublisherSources.map(source => {
      const match = collectionPublisherSources.find(({ abbr }) => abbr === source)
      return  match ? match : { abbr: source, name: source, logo: null }
    })
  , [availablePublisherSources])

  const optionsPublisherSources = coerceSourcesToComboboxOptions();
  const selectedOption = optionsPublisherSources.find((item) => item.abbr === publisherSource)
    ...
    >
      <Combobox.Target>
        <InputBase
          ...
          type="button"
          pointer
          rightSection={fetchingAvailablePublisherSources ? <Loader color="var(--bg)" size={18} /> : <Combobox.Chevron />}
          onClick={() => combobox.toggleDropdown()}
          rightSectionPointerEvents="none"
          styles={{ root: { width: 300, height: 60 }, input: { background: "var(--fg)", color: "var(--bg)", width: 300, height: 60 }  }}
          multiline
        >
          {selectedOption ? (
      ...
      <Combobox.Dropdown >
        <Combobox.Options>
          {fetchingAvailablePublisherSources ?
            <Combobox.Empty style={{ padding: "var(--medium-spacing)"}}>Loading....</Combobox.Empty>
            :
...
export function TextCount(props: TextProps) {
  ...
  return (
    <Text c="var(--mantine-color-gray-3)" size="sm" {...props}>There are {parsedSentences.length} sentence{parsedSentences.length === 1 ? null : "s"} to be uploaded</Text>
  )
}
  ...
  return (
    <Flex>
        <TextInput
          ...
          value={value}
          onChange={(event) => { setValue(event.target.value); setState({ publisherSource: event.target.value }) }}
          styles={{ input: { height: 60, width: 300 }, root: { height: 60, width: 300 }}}
        />
    </Flex>
  )
}
