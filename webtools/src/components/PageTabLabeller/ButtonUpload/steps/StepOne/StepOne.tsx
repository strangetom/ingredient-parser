...
import { Box, Center, Flex, Group, Loader, SegmentedControl, Text, Textarea } from "@mantine/core"
...
export function StepOne(){
  ...
  const onDropHandler = (files: FileWithPath[]) => {
    ...
    reader.onloadend = () => {
      timeoutRef.current = window.setTimeout(() => setState({ importingRawText: false }), 600)
    }
  ...
  const onUploadOptionHandler = (value: string) => setState({ uploadOption: value as string })
  const onRawTextChangeHandler = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
                  ...
                  <Box style={{ height: 30, width: 30 }}>
                      {importingRawText ? (
                        ...
                        <>
                          ...
                          <Dropzone.Idle>
                            <IconFileTextFilled size={30} color="var(--fg)" stroke={1.5} />
                          </Dropzone.Idle>
