...
import { Box, Button, ButtonProps, Drawer, DrawerOverlayProps, DrawerProps, Group, Stepper } from "@mantine/core"
import { useDisclosure } from "@mantine/hooks";
import { useShallow } from "zustand/react/shallow";
// {{{EXTERNAL}}}
import { Sectionable } from "../../Shared";
import { useTabLabellerStore, useUploadNewLabellersStore, TOTAL_UPLOAD_STEPS } from "../../../domain";
...
function UploadMountListener(){
  const {
    ...
    useShallow((state) => ({
      ...
      onIngredientSentenceEntryHandler: state.onIngredientSentenceEntryHandler
    })),
  )
...
export function ButtonUpload(props: ButtonProps){
  ...
  return (
    <>
      ...
      <UploadSheet
        footer={
          <Group gap="var(--xsmall-spacing)" style={{ width: "100%", justifyContent: "flex-end"}}>
            <Button variant="dark" onClick={close}>Cancel</Button>
            {activeStep !== 0 && <Button variant="light" onClick={onPrevStepHandler}>Back</Button>}
            {activeStep !== TOTAL_UPLOAD_STEPS - 1 && <Button variant="light" onClick={onNextStepHandler} loading={preuploading} disabled={rawText.length === 0}>Next</Button>}
            {activeStep === TOTAL_UPLOAD_STEPS - 1 && <Button variant="light" disabled={!publisherSource} onClick={onUploadBulkApiHandler}>Upload entries</Button>}
          </Group>
        }
        opened={opened}
        onClose={close}
      >
        <Stepper
          active={activeStep}
          onStepClick={(stepIndex: number) => setState({ activeStep: stepIndex })}
          classNames={{
