...
import { ActionIcon, Avatar, Box, Center, Flex, Group, Image, Loader, Paper, ScrollArea, Stack, Text } from "@mantine/core"
import { useShallow } from "zustand/react/shallow"
// {{INTERNAL}}
import { useTabLabellerStore, collectionPublisherSources } from "../../../domain"
// {{ASSETS}}
...
export function PrefabExamplesLabellerSource({
  ...
  return (
    <Box style={{borderRadius: "var(--xsmall-spacing)", overflow: "hidden"}}>
      {
        fetchingAvailablePublisherSources ?  (
            <Flex w={350} h={150} bg="color-mix(in srgb,var(--fg), transparent 90%)">
              <Loader color="var(--fg)" size={16} />
            </Flex>
        ) : (
          <ScrollArea.Autosize w={350} mah={150} type="always">
            <Stack gap={1}>
              {optionsPublisherSources.map(({ abbr, logo, name }, i) =>
              <Group bg="color-mix(in srgb,var(--fg), transparent 90%)" pr="var(--medium-spacing)"  pl="var(--xsmall-spacing)" py="var(--xsmall-spacing)" justify="space-between" wrap="nowrap">
                <Group wrap="nowrap" >
                  {logo ? (
                  ...
                  <Text variant="light" lh={1}>{name}</Text>
                </Group>
