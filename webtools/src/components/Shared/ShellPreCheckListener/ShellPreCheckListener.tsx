...
export function ShellPreCheckListener() {
	const { precheckPackages, prechecking, precheckValid } = useAppShellStore(
		useShallow((state) => ({
	...
	useEffect(() => {
		const fetchPreCheck = useAppShellStore.getState().fetchPreCheck;
		fetchPreCheck();
	}, []);
			...
			<div>
				<Box p="md">
					...
					<Text variant="light" lh={1.3}>
						Before you can use the application, please install the missing
						python packages found in requirements-dev.txt.
					</Text>
					<Box mt="xs" className={classes.codeRoot}>
						{precheckPackages.checks.failed.map((pckg) => (
							<Box key={`package-failed-${pckg}`} className={classes.codeLine}>
								{pckg}
							</Box>
						))}
					</Box>
				...
				<Box p="md">
					...
					<Box mt="xs" className={classes.codeRoot}>
						{precheckPackages.checks.passed.map((pckg) => (
							<Box key={`package-passed-${pckg}`} className={classes.codeLine}>
								{pckg}
							</Box>
						))}
					</Box>
