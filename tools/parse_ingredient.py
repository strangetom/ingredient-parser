...
def execute_model(features, model):
    ...
    with tempfile.NamedTemporaryFile(mode="w") as f:
        f.write(features)
        f.flush()
        return subprocess.check_output(["crf_test", "--model", model, f.name]).decode(
            "utf-8"
        )
...
def replace_fractions(string):
    ...
    """
    matches = re.findall(
        r"(?P<full>(?:(?P<int>(?:\d+)\s+)?(?P<fraction>(?:\d+/\d+))))", string
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--model", default="./models/model.crfmodel", help="Path to model"
    )
